...
class ConstitutionValidator {
  ...
  async validateArticle3() {
    ...
    if (testFileCount === 0) {
      ...
      result.warnings.push(
        `Article III: ${testFileCount} test files found - verify Red-Green-Blue cycle in git history`
      );
    }
  ...
  async validateArticle4() {
      ...
      if (await fs.pathExists(reqPath)) {
        ...
        if (hasEarsFormat) {
          ...
          result.violations.push(
            `Article IV: Requirements file exists but no EARS patterns found: ${reqPath}`
          );
          result.passed = false;
        }
        break;
    ...
    if (!requirementsFound) {
      result.warnings.push(
        'Article IV: No requirements.md found - create when starting new features'
      );
    }
  ...
  async validateArticle7() {
    ...
    if (projectCount > 3) {
      ...
      if (await fs.pathExists(phaseGatesPath)) {
        ...
        if (content.includes('Gate #001') && content.includes('APPROVED')) {
          gateApproved = true;
          result.warnings.push(
            `Article VII: ${projectCount} sub-projects detected - Phase -1 Gate #001 APPROVED`
          );
        }
      }
      ...
      if (!gateApproved) {
        result.passed = false;
        result.violations.push(
          `Article VII: ${projectCount} sub-projects detected (limit: 3) - Phase -1 Gate approval required`
        );
      }
    } else {
      result.warnings.push(
        `Article VII: ${projectCount} sub-project(s) detected - within Simplicity Gate limit`
      );
    }
    ...
    result.summary =
      projectCount > 3
        ? result.passed
          ? `Article VII: ${projectCount} sub-projects (Gate APPROVED)`
    ...
    if (abstractionCount > 0) {
      ...
      if (await fs.pathExists(phaseGatesPath)) {
        ...
        if (content.includes('Gate #002') && content.includes('APPROVED')) {
          gateApproved = true;
          result.warnings.push(
            `Article VIII: ${abstractionCount} abstraction layers - Phase -1 Gate #002 APPROVED`
          );
        }
      }
      ...
      if (!gateApproved) {
        result.warnings.push(
          `Article VIII: ${abstractionCount} potential abstraction layers detected - verify necessity`
        );
      }
    }
  ...
  async validateArticle9() {
    ...
    if (integrationTestCount > 0) {
      result.warnings.push(
        `Article IX: ${integrationTestCount} integration test files found - verify real services usage`
      );
    } else {
      result.warnings.push(
        'Article IX: No integration tests found - add when integrating external services'
      );
    }
    ...
    for (const file of files) {
      ...
      if (lines > 1500) {
        ...
        results.violations.push(
          `${path.relative(this.projectRoot, file)}: ${lines} lines (limit: 1500)`
        );
      } else if (lines > 1000) {
        results.warnings.push(
          `${path.relative(this.projectRoot, file)}: ${lines} lines (approaching limit)`
        );
      }
